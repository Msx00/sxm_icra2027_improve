...
class DistributionalSurfaceSplat(nn.Module):
    ...
    def _projection(
        ...
        finite = torch.isfinite(point_target).all(dim=0) & (
            z_target >= self.config.min_depth_mm
        )
        # Never divide by an invalid camera-plane depth and mask afterwards:
        # although indexing removes that point in the forward pass, 0/z**2 in
        # division backward becomes 0/0 at z=0 and contaminates cycle gradients.
        safe_z_target = torch.where(finite, z_target, torch.ones_like(z_target))
        u_target = (
            target_k[0, 0] * x_target / safe_z_target + target_k[0, 2]
        )
        v_target = (
            target_k[1, 1] * y_target / safe_z_target + target_k[1, 2]
        )
        finite = finite & (
