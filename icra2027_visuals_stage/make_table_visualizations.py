...
def select_table2(
    ...
    """Select two positive-margin examples per dataset from distinct keyframes."""
    ...
    for dataset_prefix in ("endovis_dataset_8", "endovis_dataset_9"):
        ...
        first = candidates[0]
        second = next((row for row in candidates[1:] if row["scene"] != first["scene"]), None)
        if second is None:
            raise ValueError(f"No distinct-keyframe second sample for {dataset_prefix}")
...
def draw_table1(output_dir: Path) -> tuple[Path, list[dict[str, Any]], list[dict[str, str]]]:
    ...
    row_labels = [
        (
            f"S{row['scene'].split('_')[1]}-{row['scene'].split('_')[3]}\n"
            f"#{int(row['frame_id'])}\n{100.0 * float(row['raw_hole_ratio']):.1f}% hole"
        )
        for row in selected
    ]

    fig = plt.figure(figsize=(7.16, 3.38))
    ...
    add_panel_heading(fig, bottom_axes, "(b) Image completion")
...
def draw_table2(output_dir: Path) -> tuple[Path, list[dict[str, Any]], list[dict[str, str]]]:
    ...
    for sample in selected:
        scene_parts = sample["scene"].split("_")
        dataset_id = scene_parts[2]
        keyframe_id = scene_parts[-1]
        row_labels.append(
            f"Dataset {dataset_id}\nKF{keyframe_id} #{frame_number(sample)}\n"
            f"{100.0 * float(sample['raw_hole_ratio']):.1f}% hole"
        )

    fig = plt.figure(figsize=(7.16, 3.58))
        ...
        for column_index, label in enumerate(columns):
            ...
            plot_cell(
                ...
                metadata={
                    ...
                    "selection_rule": "largest positive DistriNVS PSNR margin per dataset, raw hole >= 0.10, two distinct keyframe sequences",
                },
...
def main() -> None:
    ...
    check_manifest(table1_manifest, 34, "Table-I qualitative figure")
    check_manifest(table2_manifest, 32, "Table-II qualitative figure")
    ...
    provenance = {
        ...
        "table2": {
            ...
            "selection_rule": "largest positive DistriNVS PSNR margin per dataset with raw hole >= 0.10 and two distinct keyframe sequences",
