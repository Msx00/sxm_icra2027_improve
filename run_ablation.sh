...
TRAIN_GPU_GROUPS="${TRAIN_GPU_GROUPS:-0,1,2,3;0,1,2,3}"
...
usage() {
  cat <<'EOF'
Run reproducible DistriSurg ablations, with parallel training workers.
...
Environment variables:
  ...
  TRAIN_GPU_GROUPS='0,1,2,3;0,1,2,3'
                                Semicolon-separated GPU groups; one task per group
...
Examples:
  ...
  SUITE=core TRAIN_GPU_GROUPS='0,1,2,3;0,1,2,3' bash run_ablation.sh
...
IFS=';' read -r -a train_gpu_groups <<< "$TRAIN_GPU_GROUPS"

for group in "${train_gpu_groups[@]}"; do
  [[ "$group" =~ ^[0-9]+(,[0-9]+)*$ ]] || {
    echo "Invalid GPU group '$group' in TRAIN_GPU_GROUPS=$TRAIN_GPU_GROUPS" >&2
    exit 2
  }
  IFS=',' read -r -a group_gpu_ids <<< "$group"
  declare -A group_seen_gpu_ids=()
  for gpu_id in "${group_gpu_ids[@]}"; do
    [[ ! -v "group_seen_gpu_ids[$gpu_id]" ]] || {
      echo "GPU $gpu_id occurs more than once in GPU group '$group'" >&2
      exit 2
    }
    group_seen_gpu_ids[$gpu_id]=1
  done
  unset group_seen_gpu_ids
done
...
train_one() {
  local name="$1" seed="$2" cfg="$3" output="$4" gpu_ids="$5"
  ...
  echo "[train] $name seed=$seed gpus=$gpu_ids config=${cfg:-full} output=$output"
  if [[ "$DRY_RUN" == "1" ]]; then
...
run_parallel_training() {
  local worker_count="${#train_gpu_groups[@]}" worker job_idx failed=0 worker_pid
  local -a worker_pids=()
  echo "[train pool] workers=$worker_count gpu_groups=$TRAIN_GPU_GROUPS jobs=${#job_names[@]}"
  for ((worker = 0; worker < worker_count; worker++)); do
    (
      for ((job_idx = worker; job_idx < ${#job_names[@]}; job_idx += worker_count)); do
        train_one "${job_names[$job_idx]}" "${job_seeds[$job_idx]}" \
          "${job_configs[$job_idx]}" "${train_outputs[$job_idx]}" \
          "${train_gpu_groups[$worker]}"
      done
    ) &
    worker_pids+=("$!")
  done
  ...
  [[ "$failed" == "0" ]] || {
    echo "One or more parallel training workers failed" >&2
    return 1
  }
}
...
if [[ "$PHASE" == "eval" || "$PHASE" == "all" ]]; then
  for ((job_idx = 0; job_idx < ${#job_names[@]}; job_idx++)); do
    eval_one "${job_names[$job_idx]}" "${job_seeds[$job_idx]}" \
      "${job_configs[$job_idx]}" "${train_outputs[$job_idx]}/latest.pt" \
      "${eval_outputs[$job_idx]}"
  done
fi
