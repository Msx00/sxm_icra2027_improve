...
class VisibilityRouter(nn.Module):
    ...
    def forward(
        ...
        # Optimize the router in logit space. Probability-space BCE has an
        # unbounded derivative near 0/1; with AMP its scaled gradient can
        # overflow and poison the following optimizer step. Adding a bounded
        # residual to the prior logit preserves the intended initialization.
        prior_probability = physics_prior.clamp(1.0e-4, 1.0 - 1.0e-4)
        # The physics prior is an analytic routing constraint, not a trainable
        # shortcut into DSS. Detaching it also prevents the 1/[p(1-p)] logit
        # derivative near 0/1 from destabilizing geometry gradients.
        physics_logit = torch.logit(prior_probability.detach())
        residual = self.config.router_residual_scale * torch.tanh(
            self.learned_residual(condition)
        )
        synthesis_logits = physics_logit + residual
        synthesis_gate = torch.sigmoid(synthesis_logits)
        trusted = (
            valid_mask.bool()
